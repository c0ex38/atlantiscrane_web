        ...
        <div className="flex w-full flex-col md:flex-row">
          {items.map((item, index) => {
            ...
            return (
              <article
                key={item.title}
                className="group relative h-[380px] flex-1 overflow-hidden transition-[flex] duration-700 ease-in-out sm:h-[500px] md:hover:flex-[1.6] lg:h-[580px] lg:hover:flex-[1.8]"
              ...
              >
                ...
                {/* Overlays */}
                <div 
                  className="absolute inset-0 bg-slate-900/40 transition-opacity duration-[1200ms] group-hover:opacity-0" 
                  style={{ willChange: "opacity" }}
                />
                <div 
                  className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-80 transition-opacity duration-[1200ms] group-hover:opacity-100" 
                  style={{ willChange: "opacity" }}
                />

                {/* Giant Number Background */}
                <div 
                  className="absolute -bottom-10 right-4 z-0 pointer-events-none transition-transform duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:-translate-y-8 group-hover:scale-110 md:-bottom-20 md:-right-10"
                  style={{ willChange: "transform" }}
                >
                  <span 
                ...
                <div className="absolute inset-x-0 bottom-0 flex h-full flex-col justify-end p-7 sm:p-8 lg:p-10">
                  <div className="relative z-10 w-full md:w-[320px]">
                    <h3 
                      className="text-2xl font-black uppercase leading-[0.95] tracking-tight text-white drop-shadow-xl sm:text-3xl transition-transform duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:-translate-y-2"
                      style={{ willChange: "transform" }}
                    >
                      {item.title}
                    </h3>
                    <div 
                      className="mt-5 h-1.5 w-20 origin-left bg-[color:var(--cta)] transition-transform duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] scale-x-50 group-hover:scale-x-100 group-hover:-translate-y-2"
                      style={{ willChange: "transform" }}
                    />
                    ...
                    <div className="grid grid-rows-[0fr] transition-[grid-template-rows] duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:grid-rows-[1fr]">
                      <div className="overflow-hidden">
                        <div 
                          className="transform translate-y-6 pt-6 opacity-0 transition-[transform,opacity] delay-150 duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:translate-y-0 group-hover:opacity-100"
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="flex h-6 w-6 items-center justify-center rounded-full border border-[color:var(--cta)] transition-[background-color,color,transform] group-hover/link:bg-[color:var(--cta)] group-hover/link:text-slate-900 group-hover/link:translate-x-1">
                              →
                            </span>
                          </Link>
                        </div>
