...
export function AdminPageShell({
  ...
  return (
    <div className={`mx-auto w-full max-w-6xl space-y-5 animate-in fade-in duration-300 ${className}`}>
      {children}
    </div>
  );
}
...
export function AdminLanguageTabs({
  ...
  return (
    <div
      ...
      className="flex self-start rounded-lg border border-border/60 bg-muted/50 p-0.5 sm:self-auto"
    >
  ...
  return createPortal(
    ...
    >
      <form
        ...
        className="flex max-h-[calc(100dvh-1rem)] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-border/70 bg-background shadow-2xl sm:max-h-[calc(100dvh-2rem)]"
      >
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-border/70 bg-card px-5 py-4 sm:px-6">
          <div className="min-w-0">
        ...
        <footer className="flex shrink-0 items-center justify-end gap-2 border-t border-border/70 bg-card px-5 py-4 sm:px-6">
          <button
            type="button"
            onClick={onClose}
...
export const adminPrimaryButtonClass =
  "flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-xs font-bold text-primary-foreground shadow-md shadow-primary/20 transition-all hover:bg-primary/90 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50";
